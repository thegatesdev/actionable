...
public final class EntityLocationActions extends StaticFactoryRegistry<Consumer<Twin<Entity, Location>>, ActionFactory<Twin<Entity, Location>>> {
    ...
    public void registerStatic() {
        register(ActionFactory.moreFactory(ENTITY_LOCATION_ACTION));
        ...
        register(new ActionFactory<>("teleport_to", (data, twin) -> twin.actor().teleport(twin.target().setDirection(twin.actor().getLocation().getDirection()).add(data.getObject("offset", Vector.class))), new ReadableOptions().add("offset", VECTOR, new Vector(0, 0, 0))));
